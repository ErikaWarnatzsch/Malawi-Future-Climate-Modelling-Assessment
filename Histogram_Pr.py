...
def main():
    ...
    #Since 1 kg of rain spread over 1 m of surface is 1mm in thickness, and there are 60 seconds in a minute, 60 minutes in a hour, 24 hours in a day, the conversion is: 
    Convert=60*60*24
    ...
    CRU.units = Unit('mm per day')
    UDel.units = Unit('mm per day')
    GPCC.units = Unit('mm per day')
    ...
    GPCC = GPCC.aggregated_by('year', iris.analysis.MEAN)
    ...
    GPCC_mean = GPCC_mean.collapsed(['time'], iris.analysis.MEAN)    
    
    #create average of observed baseline data
    Obs = (CRU_mean + UDel_mean + GPCC_mean)/3
    ...
    #PART 7: PRINT DATA
    import csv
    with open('output_Pr.csv', 'wb') as csvfile:
